...
from source import models, losses, datasets
...
if __name__ == '__main__':
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset', default='MpiSintelFinal',
                                   ...
                                   parameter_defaults={'root': './dataset/MPI-Sintel/training'})
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.inference_dataset_root):
            ...
            # corresponding ground_truth flow field (not exist during test set)
    ...
    def test(args, epoch, data_loader, model, offset=0):
        ...
        for batch_idx, data in enumerate(progress):
            i = data[0].size()
            target = torch.FloatTensor(i).zero_()
            target = target[:,0,:,:]
            if args.cuda:
                data, target = [d.cuda(async=True) for d in data], [t.cuda(async=True) for t in target]
            data, target = [Variable(d, volatile=True) for d in data], [Variable(t, volatile=True) for t in target]
    ...
    for epoch in progress:
        if args.test:
            stats = test(args=args, epoch=epoch - 1, data_loader=inference_loader, model=model_and_loss,
                              offset=offset)
            offset += 1
