...
class DAVIS2017FlowNet2(data.Dataset):
    def __init__(self, args, is_cropped = False, root = '', dstype = '480p', replicates = 1):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
        ...
        file_list = sorted(glob(join(image_root, '*/*.jpg')))
        ...
        for file in file_list:
            ...
            img1 = join(image_root, fprefix + "%04d"%(fnum+0) + '.jpg')
            img2 = join(image_root, fprefix + "%04d"%(fnum+1) + '.jpg')
